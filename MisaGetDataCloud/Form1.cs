                    ...
                    foreach (var item in rs.result)
                    {
                        ...
                        else if (Selected == 1)
                        {
                            ...
                            foreach (var dv in hanghoadichvus)
                            {
                                ...
                                {
                                    bool checklist = true;
                                    foreach (var dt in from dt in dtDetailDoiTuongThu// kiểm tra đã tồn tại loại đối tượng trước đó
                                                       where dv.TenLoaiDoiTuongThu.Equals(dt.Description)
                                                       select dt)
                                    {
                            ...
                            foreach (var dv in hanghoadichvus)
                            {
                                if (dv.TienBNCCT > 0)
                                {
                                    ...
                                    if (dv.TienBNTT > 0)
                                    {
                                        ...
                                        {
                                            bool Checklist = true;
                                            foreach (var dt in from dt in dtDetailDoiTuongThu
                                                               where dv.TenLoaiDoiTuongThu.Equals(dt.Description)
                                                               select dt)
                                            {
